...
class ProcessManagerTests(unittest.TestCase):
    def test_services_defined(self) -> None:
        ...
        ee = next(s for s in SERVICES if s.key == "event-engine")
        self.assertEqual(ee.log_name, "event-intelligence.log")
        self.assertIn("event-engine-run", ee.module_args)
        ms = next(s for s in SERVICES if s.key == "multi-source")
        self.assertEqual(ms.log_name, "multi-source.log")
        self.assertIn("multi-source-run", ms.module_args)
