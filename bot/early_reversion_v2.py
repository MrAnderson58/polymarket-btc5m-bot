...
from bot.no_c_btc_filter import (
    log_no_c_filter_mode,
    log_no_c_filter_skipped,
    record_no_c_filter_normal_entry,
    record_no_c_filter_strict_entry,
    record_no_c_filter_skipped_strict_price,
    resolve_no_c_entry_threshold,
    resolve_no_c_filter,
)
from bot.no_c_filter_shadow import NO_C_STRATEGY, record_no_c_filter_shadow
...
def _close_trade(
    ...
    *,
    ...
    def _finalize() -> None:
        ...
        record_exit(conn, "v2", trade["strategy_name"], exit_reason)
        close_early_reversion_v2_trade(
            conn,
            trade["id"],
            exit_price=bid,
...
def _log_entry_diagnostics(
    ...
    *,
    ...
    diagnostics = evaluate_version_entries(
        conn,
        strategy_version="v2",
        active_signals=ACTIVE_SIGNALS_V2,
        market_slug=market.slug,
        seconds_open=seconds_open,
        entry_window_sec=ER_V2_ENTRY_WINDOW_SEC,
        quotes=quotes,
        has_trade_fn=has_early_reversion_v2_trade,
        btc_price=btc_price,
        now_ts=now_ts,
    )
    return log_version_entry_check("V2", diagnostics), diagnostics
...
def _try_open_signals(
    ...
    *,
    ...
    log_pipeline(7, detail=f"active_signals={len(ACTIVE_SIGNALS_V2)}")
    if not ACTIVE_SIGNALS_V2:
        log_pipeline(8)
    ...
    for signal in ACTIVE_SIGNALS_V2:
        ...
        if signal.strategy_name == NO_C_STRATEGY and btc_price is not None:
            filter_decision = resolve_no_c_filter(
                conn,
                current_btc=btc_price,
                now_ts=now_ts,
            )
            log_no_c_filter_mode(filter_decision)
            threshold = filter_decision.threshold
        else:
        ...
        if ask > threshold:
            if filter_decision is not None and filter_decision.mode == "STRICT":
                log_no_c_filter_skipped(filter_decision, ask=ask)
            ...
            pipeline_stop_for_diagnostic(
                ...
                detail=f"ask={ask} threshold={threshold:.2f}",
            )
            continue
        ...
        if (
            signal.strategy_name == NO_C_STRATEGY
            and ENABLE_NO_C_FILTER_SHADOW
        ...
        if filter_decision is not None:
            if filter_decision.mode == "NORMAL":
                record_no_c_filter_normal_entry(conn)
            else:
                record_no_c_filter_strict_entry(conn)
        logger.info(
...
def process_early_reversion_v2(
    ...
    *,
    ...
    signal_status, diagnostics = _log_entry_diagnostics(
        conn,
        market,
        quotes,
        seconds_open=seconds_open,
