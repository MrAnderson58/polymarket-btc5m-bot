...
def _resolve_exit_decision(
    *,
    ...
    reason = resolve_er_exit_reason(
        entry_price=entry_price,
        bid=bid,
        max_price_seen=max_price_seen,
        seconds_in_trade=seconds_in_trade,
        grace_sec=0,
        stop_loss_pct=ER_V3_STOP_LOSS_PCT,
        time_stop_sec=ER_V3_TIME_STOP_SEC,
        legacy_trailing_pct=ER_V3_TRAILING_STOP_PCT,
        legacy_trailing_grace_sec=ER_V3_TRAILING_GRACE_SEC,
        trailing_active=trailing_active,
        highest_after_activation=highest_after_activation,
    )
    if reason is None:
        return None
    stop_loss_trigger_pnl = None
    if reason == "STOP_LOSS":
        stop_loss_trigger_pnl = _pnl_percent(entry_price, bid)
    return ExitDecision(reason=reason, stop_loss_trigger_pnl=stop_loss_trigger_pnl)
...
def _close_trade(
    ...
    *,
    ...
    def _finalize() -> None:
        ...
        record_exit(conn, "v3", trade["strategy_name"], decision.reason)
        close_early_reversion_v3_trade(
            conn,
            trade["id"],
            exit_price=bid,
...
def _log_entry_diagnostics(
    ...
    *,
    ...
    diagnostics = evaluate_version_entries(
        conn,
        strategy_version="v3",
        active_signals=ACTIVE_SIGNALS_V3,
        market_slug=market.slug,
        seconds_open=seconds_open,
        entry_window_sec=ER_V3_ENTRY_WINDOW_SEC,
        quotes=quotes,
        has_trade_fn=has_early_reversion_v3_trade,
    )
    return log_version_entry_check("V3", diagnostics), diagnostics
...
def _try_open_signals(
    ...
    *,
    ...
    log_pipeline(7, detail=f"active_signals={len(ACTIVE_SIGNALS_V3)}")
    if not ACTIVE_SIGNALS_V3:
        log_pipeline(8)
