...
from bot.research.mtf.research_1h.config import (
    FIXED_SL_PCT, FIXED_TP_PCT, TIME_EXIT_SECONDS_LEFT,
    TRAIL_ACTIVATE_PCT, TRAIL_DISTANCE_PCT,
)
from bot.research.mtf.research_1h.models import Obs1h, SimTrade1h
...
def _entry(obs: Obs1h, side: str):
    return obs.yes_ask if side == "YES" else obs.no_ask


def settlement(obs: Obs1h, side: str) -> float:
...
def try_entry(path: list[Obs1h], idx: int, side: str) -> SimTrade1h | None:
    obs = path[idx]
    ep = _entry(obs, side)
    if ep is None or ep <= 0:
        return None
    if obs.entry_second < 1200:
        regime = "early"
    elif obs.entry_second < 2400:
        regime = "mid"
    else:
        regime = "late"
    if obs.vol_expansion >= 1.3:
        regime = f"{regime}_high_vol"
    return SimTrade1h("", side, obs.market_slug, obs.timestamp, ep, obs.entry_second, regime=regime)


def simulate_exit(trade: SimTrade1h, path: list[Obs1h], start_idx: int, mode: str) -> SimTrade1h:
    ...
    for j in range(start_idx + 1, len(path)):
        ...
        if mode == "prob_reversal" and obs.yes_mid is not None:
            if (side == "YES" and obs.yes_mid < 0.47) or (side == "NO" and obs.yes_mid > 0.53):
