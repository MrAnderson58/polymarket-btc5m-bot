...
def render_1h_report(result: dict[str, Any]) -> str:
    lines = ["BTC 1H POLYMARKET BIDIRECTIONAL RESEARCH", "=" * 50, ""]
    ...
    if result.get("thresholds"):
        lines.append("TRAIN THRESHOLDS (1h-specific)")
        for k, v in result["thresholds"].items():
            lines.append(f"  {k}: {v:.2f}")
        lines.append("")

    lines.append(f"{'family':<22} {'side':<5} {'exit':<14} {'N':>4} {'PF':>6} {'OOS':>6} {'WR':>6}")
    lines.append("-" * 70)
