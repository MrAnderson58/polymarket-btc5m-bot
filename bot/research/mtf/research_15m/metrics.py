"""Performance metrics for 15m research."""
...
from bot.research.mtf.research_15m.config import BOOTSTRAP_SAMPLES, STRESS_LEVELS
from bot.research.mtf.research_15m.models import SimTrade
...
def apply_stress(trades: list[SimTrade], slip: float) -> list[float]:
    """Worse entry fill: pay +slip on entry probability."""
    out = []
    for t in trades:
        ...
        stressed_entry = min(0.99, entry + slip)
        if t.side == "YES":
            out.append((exit_p - stressed_entry) / stressed_entry * 100.0)
        else:
            out.append((exit_p - stressed_entry) / stressed_entry * 100.0)
    return out
