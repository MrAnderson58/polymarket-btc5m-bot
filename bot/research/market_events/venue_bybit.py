...
class BybitMarketClient:
    def __init__(self, *, api_base: str = BYBIT_API, timeout: float = DEFAULT_TIMEOUT) -> None:
        self._api = api_base.rstrip("/")
        self._timeout = timeout

    def _get(self, path: str, params: dict[str, Any]) -> dict[str, Any]:
        r = requests.get(f"{self._api}{path}", params=params, timeout=self._timeout)
        r.raise_for_status()
        data = r.json()
    ...
    def fetch_ticker(self, symbol: str, *, category: str = "linear") -> BybitTicker | None:
        try:
            ...
            logger.warning("Bybit ticker failed %s: %s", symbol, exc)
            return None
