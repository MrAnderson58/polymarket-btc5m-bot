...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in (
        ...
        from bot.research.market_events.signal_intelligence.elite_profile_audit_v1 import (
        ...
        try:
            ...
            def _run_epa() -> dict:
                with research_write_connection(db_path) as conn:
                    apply_migrations(conn)
                    if args.command == "elite-profile-verify":
                        return run_elite_profile_verify(conn)
                    if args.command == "elite-profile-bias":
                        return run_elite_profile_bias(conn)
    ...
    if args.command in (
        ...
        try:
            ...
            def _run_pm() -> dict:
                with research_write_connection(db_path) as conn:
                    apply_migrations(conn)
                    if args.command == "paper-math-review":
                        return run_paper_math_review(conn)
                    if args.command == "paper-math-report":
                        return run_paper_math_report(conn, write_reports=True)
    ...
    if args.command == "research-integrity":
        from bot.research.market_events.db import retry_on_db_locked
        from bot.research.market_events.research_db_session import research_write_connection
        from bot.research.market_events.research_sync_v1 import (
            ResearchSyncError,
        ...
        try:
            ...
            def _run_ri() -> dict:
                with research_write_connection(db_path) as conn:
                    apply_migrations(conn)
                    return run_research_integrity_v1(conn, write_reports=True)

            out = retry_on_db_locked(_run_ri)
            print(out.get("terminal") or "")
            return 0 if out.get("ok") else 1
        except Exception as exc:
            print(f"research-integrity failed: {exc}", file=sys.stderr)
    ...
    if args.command == "replay-recovery":
        from bot.research.market_events.db import retry_on_db_locked
        from bot.research.market_events.research_db_session import research_write_connection
        from bot.research.market_events.research_sync_v1 import (
            ResearchSyncError,
        ...
        from bot.research.market_events.signal_intelligence.replay_recovery_v1 import (
        ...
        try:
            ...
            def _run_rr() -> dict:
                with research_write_connection(db_path) as conn:
                    apply_migrations(conn)
                    return run_replay_recovery_v1(conn, write_reports=True)

            out = retry_on_db_locked(_run_rr)
            print(out.get("terminal") or "")
            return 0 if out.get("ok") else 1
        except Exception as exc:
            print(f"replay-recovery failed: {exc}", file=sys.stderr)
