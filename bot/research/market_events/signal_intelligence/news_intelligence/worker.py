...
WORKER_POLL_SEC = 60
LOG_PATH = BASE_DIR / "logs" / "news-intelligence.log"
_shutdown = False
...
def _configure_logging() -> None:
    ...
    root = logging.getLogger(
        "bot.research.market_events.signal_intelligence.news_intelligence"
    )
    if any(
    ...
    root.addHandler(fh)
    # Also log collector / N11 under same file for ops visibility.
    n11 = logging.getLogger(
        "bot.research.market_events.signal_intelligence.news_collector_n11"
    )
    n11.setLevel(logging.INFO)
    if not any(
        isinstance(h, logging.FileHandler)
        and Path(getattr(h, "baseFilename", "")).resolve() == LOG_PATH.resolve()
        for h in n11.handlers
    ):
        n11.addHandler(fh)
...
def run_news_intelligence_worker_s41(
    *,
    ...
    logger.info(
        "news-intel-worker started agg=%ss brief=%ss poll=%ss",
        aggregation_interval_sec,
        brief_interval_sec,
        poll_sec,
    )
    print(
    ...
    while not _shutdown:
        ...
        try:
            if now - last_agg >= int(aggregation_interval_sec):
                ...
                aggregations += 1
                logger.info(
                    "aggregation summaries=%s deduped=%s",
                    agg.get("summaries_written"),
                    agg.get("deduped"),
                )
                print(
    ...
    logger.info("news-intel-worker stopped %s", stats)
