...
def _ensure_table(conn: Any) -> None:
    ...
    if row is None:
        raise RuntimeError(
            "market_news_feed_n11 missing — run: "
...
def insert_news_item_n11(conn: Any, item: dict[str, Any], *, now: int | None = None) -> int | None:
    ...
    try:
        ...
        symbols_str = ",".join(tagged["symbols"])[:200] if tagged["symbols"] else symbols_str
    except Exception:
    ...
    # Prefer enriched columns when migration S411 applied; fall back otherwise.
    try:
        cur = conn.execute(
            """
            INSERT INTO market_news_feed_n11 (
                published_at, source, title, summary, url, symbols, category, raw_json, created_at,
                source_type, importance, language, body
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
